...
function getCustomerProfile(customerId) {
  ...
  const idxMap = _mapHeaders(headers, 'getCustomerProfile'); // Helper function untuk map index kolom
...
function processTopUp(form) {
  ...
  try {
    ...
    const idxMap = _mapHeaders(headers, 'processTopUp: Data_Pelanggan');
...
function simpanOrderBaru(form) {
  ...
  try {
    ...
    const pIdx = _mapHeaders(pHeaders, 'simpanOrderBaru: Data_Pelanggan');
...
// --- INTERNAL HELPER ---
function _mapHeaders(headersArray, contextMessage) {
  if (!Array.isArray(headersArray) || !headersArray.length) {
    const ctx = contextMessage ? ` (${contextMessage})` : '';
    throw new Error(`Header tidak ditemukan${ctx}.`);
  }
  ...
  headersArray.forEach((h, i) => {
    ...
    if (!key) {
      throw new Error(`Header kosong pada kolom ${i + 1}${ctx}.`);
    }

    if (map[key] !== undefined) {
      throw new Error(`Header duplikat '${key}' pada kolom ${i + 1}${ctx}.`);
    }
...
function _ensureSheet(ss, sheetName, headersArray) {
  ...
  if (sheet.getLastRow() === 0 && Array.isArray(headersArray) && headersArray.length) {
